...
public class ReaderLedControl
{
    ...
    /// <returns>An instance of ReaderLedControl representing the message payload</returns>
    public static ReaderLedControl ParseData(ReadOnlySpan<byte> data)
    {
        return new ReaderLedControl(
            data[0],
            data[1],
            (TemporaryReaderControlCode)data[2],
            data[3],
            data[4],
            (LedColor)data[5],
            (LedColor)data[6],
            Message.ConvertBytesToUnsignedShort(data.Slice(7, 2)),
            (PermanentReaderControlCode)data[9],
            data[10],
            data[11],
            (LedColor)data[12],
            (LedColor)data[13]);
